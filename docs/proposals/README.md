...
| [opencode-borrow.zh-CN.md](opencode-borrow.zh-CN.md) | 借鉴 opencode：闭合「编辑→验证」（LSP 诊断回灌）与「暂停→交互」（HITL）环路 + 反应式压缩 / web 搜索 / Plan Mode / 快照回退 | In progress — **P0 全部落地**：P0.1 HITL · P0.2 编辑后 LSP 诊断回灌（`@jarvis/lsp`，`JARVIS_ENABLE_LSP`）· P0.3 反应式压缩（`contextWindow` 派生预算 + 结构化锚点摘要）· P0.4 `http.fetch format:"markdown"`（node-html-markdown）+ web 搜索复用 MCP 桥，测试全绿；P1 / P2 pending |
