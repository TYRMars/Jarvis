...
> **✅ DONE — P8 complete.** Every gap below is closed (no remaining stubs): the
> Rust crates/apps were removed (tag `rust-archive-pre-takedown`), Node is the
> sole runtime, and the full close-out shipped — memory git/iCloud sync ported
> (`/v1/memory/sync*` + `/includes*` live), OTel tracing (8.4), Node perf
> baseline (8.3, `make perf`), security baseline + hardening (8.6,
> `docs/security/p8-security-baseline.md`), and README/ARCHITECTURE rewritten
> Node-native (8.5). The Node server passes its full test gate.
...
| `POST /v1/workspace/commit`, `GET /v1/workspace/pr/preview`, `POST /v1/workspace/pr` | web Commit/PR dialogs | M | ✅ done (git + `gh`) |
| `GET /v1/server/info` (runtime config snapshot, ~15 fields, no secrets) | web Settings·ServerSection, ContextWindowBadge, SDK | M | ✅ done (`meta-routes.ts`) |
| `POST /v1/providers/:name/probe` (connectivity ping) | web Settings·ProvidersSection | M | ✅ done (`meta-routes.ts`) |
| `GET/PUT /v1/routing` + `PATCH/DELETE /v1/routing/:slot` (ModelRoutePolicy CRUD) | web Settings·RoutingSection | S×4 | ✅ done (`routing-routes.ts`; summarization slot wired into SummarizingMemory — not hollow) |
| `GET /v1/memory/sync_status`, `POST /v1/memory/sync`, `…/sync_setup`, `…/sync_setup_icloud`; `GET/POST/DELETE /v1/memory/includes`, `POST …/includes/refresh` | web MemorySyncSection + MemoryIncludesPanel | M each | ✅ done (P8 — git/iCloud `memory.*` tools ported; `memory-sync-routes.ts`) |
| `POST /v1/requirements/:id/conversations`, `…/todos` CRUD, `POST /v1/runs/:id/verify` + `/verification` | web Projects/RequirementDetail | S–L | ✅ done (`requirements-routes.ts` + `verification.ts`) |
