...
Jarvis 现在已经具备做代码助手的核心积木：流式 agent loop、多 provider、
文件工具、正则代码搜索、`shell.exec`、审批、memory、持久化，以及一个 Web
客户端。

下一步更有价值的方向不是继续把它做成“更会聊天的助手”，而是把这些积木组合成
一个更完整的 AI Coding 工作流，接近 Codex / Claude Code 的使用体验：
...
这个 proposal 的目标，是在不破坏现有架构边界的前提下，把 Jarvis 推进成一个真正
的 coding agent。核心设计原则仍然不变：`harness-core` 只拥有 agent loop 和 trait，
不理解 Git、HTTP、存储、UI 或具体工具策略。
