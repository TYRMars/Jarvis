...
//!
//! Extracted from `harness-core` together with the channel and project
//! domains — this crate is a true leaf, no `harness-*` deps. Two
//! related trait surfaces ([`ObservabilityStore`] for ingested run /
//! span facts, [`EvalStore`] for grader-driven evaluation runs) live
//! together because they share the same "is the system doing well?"
//! audience.
...
/// `harness_core::BoxError` / `harness_channel::BoxError` /
/// `harness_project::BoxError` — all expand to the same
/// `Box<dyn std::error::Error + Send + Sync>`, fully interchangeable
/// because Rust type aliases are transparent.
