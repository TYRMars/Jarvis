...
//! kind.
//!
//! This is M1 of the Channels work: persistence + REST + Settings
//! page. M2 ships the first real `kind` (`wecom_webhook` — outbound
//! only). The trait + JSON-on-disk shape are intentionally agnostic
//! about which kinds exist so adding feishu / wechat_mp / dingtalk in
//! follow-ups is a config-schema change, not a store-schema change.
//!
