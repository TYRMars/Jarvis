...
pub async fn import_proposals(
    ...
    if let Some(rel) = opts.source_subdir.as_deref() {
        validate_source_subdir(rel)?;
    }
    ...
    // Touch the Project's updated_at only when something actually
    // changed — otherwise a cron-driven re-import would keep bubbling
    // the roadmap project to the top of the "newest-updated first"
    // sort even on no-op runs.
...
/// Reject absolute paths and any path that traverses out of the workspace.
/// Same posture as `harness_tools::sandbox::resolve_under` (which we can't
/// import here without a circular dep). Called by `import_proposals` before
/// `resolve_source` so the caller gets a clear error rather than the
/// "no source found" sentinel.
fn validate_source_subdir(rel: &Path) -> Result<(), BoxError> {
    use std::path::Component;
    if rel.is_absolute() {
        return Err(format!(
            "source_subdir `{}` must be relative to the workspace root",
            rel.display()
        )
        .into());
    }
    for c in rel.components() {
        match c {
            Component::ParentDir => {
                return Err(format!(
                    "source_subdir `{}` must not contain `..` (would escape the workspace)",
                    rel.display()
                )
                .into());
            }
            Component::Prefix(_) | Component::RootDir => {
                return Err(format!(
                    "source_subdir `{}` must be a plain relative path",
                    rel.display()
                )
                .into());
            }
...
fn resolve_source(
    ...
    if let Some(rel) = explicit {
        // Defence in depth — `import_proposals` already validates, but
        // any direct caller skipping that path still gets sandboxed.
        if validate_source_subdir(rel).is_err() {
            return None;
        }
...
fn extract_status_text(text: &str) -> Option<String> {
    ...
    while let Some(line) = lines.next() {
        ...
        while let Some(next) = lines.peek() {
            ...
            if trimmed.is_empty() || is_field_marker(trimmed) || is_heading(trimmed) {
                break;
            }
            if !buf.is_empty() {
                buf.push(' ');
...
/// `# Heading`, `## Section`, …, `###### h6` (1-6 `#` chars + space).
...
mod tests {
    ...
    #[tokio::test]
    async fn source_subdir_with_parent_dir_is_rejected() {
        let outer = tempdir().unwrap();
        let workspace = outer.path().join("jarvis");
        std::fs::create_dir_all(&workspace).unwrap();
        let (projects, requirements) = make_stores();
        let opts = ImportOptions {
            source_subdir: Some(PathBuf::from("../etc")),
            ..Default::default()
        };
        let err = import_proposals(&workspace, &projects, &requirements, opts)
            .await
            .unwrap_err();
        assert!(
            err.to_string().contains("..") || err.to_string().contains("escape"),
            "unexpected error: {err}"
        );
    }

    #[tokio::test]
    async fn source_subdir_absolute_is_rejected() {
        let outer = tempdir().unwrap();
        let workspace = outer.path().join("jarvis");
        std::fs::create_dir_all(&workspace).unwrap();
        let (projects, requirements) = make_stores();
        ...
        let err = import_proposals(&workspace, &projects, &requirements, opts)
            ...
            .unwrap_err();
        assert!(
            err.to_string().contains("relative") || err.to_string().contains("plain"),
            "unexpected error: {err}"
        );
    }

    #[tokio::test]
    async fn unchanged_rerun_does_not_touch_project_updated_at() {
        let outer = tempdir().unwrap();
        let workspace = outer.path().join("jarvis");
        let proposals = workspace.join("docs/proposals");
        std::fs::create_dir_all(&proposals).unwrap();
        ...
        // Sleep is ugly but the alternative is mocking time. Updates are
        // RFC3339 with millisecond precision, so 25ms is enough to detect
        // an unwanted touch.
        ...
        assert_eq!(
            proj_after_second.updated_at, first_updated,
            "no-op import bumped project.updated_at"
        );
    }

    #[test]
    fn extract_status_text_breaks_on_following_heading() {
        // No blank line between status and the next heading — previously
        // the heading text leaked into the status.
        let body = "# Title\n\n**Status:** Adopted\n## Next section\nBody.\n";
        let s = extract_status_text(body);
        assert_eq!(s.as_deref(), Some("Adopted"));
    }

    #[test]
    fn is_heading_detects_md_headings() {
        assert!(is_heading("# A"));
        assert!(is_heading("## B"));
        assert!(is_heading("###### F"));
        assert!(!is_heading("####### too many"));
        assert!(!is_heading("**bold**"));
