...
//! [`harness_project::requirement_run::RequirementRunEvent`] so the
//! [`harness_project::RequirementRunStore`] trait could name it without
//! pulling `harness-requirement` into `harness-core`. Imports of
//! `harness_requirement::RequirementRunEvent` still work via this
//! re-export.

pub use harness_project::requirement_run::RequirementRunEvent;
