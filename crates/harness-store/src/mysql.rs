...
async fn migrate(pool: &MySqlPool) -> Result<(), StoreError> {
    ...
    // Idempotent migration: add `assignee_id` to pre-existing tables.
    // MySQL 8.0.29+ supports `ADD COLUMN IF NOT EXISTS`, but earlier
    // versions don't — check INFORMATION_SCHEMA.COLUMNS first to stay
    // portable.
    let has_req_assignee: bool = sqlx::query_scalar::<_, i64>(
        "SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
           WHERE TABLE_SCHEMA = DATABASE()
             AND TABLE_NAME = 'requirements'
    ...
    .execute(pool)
    .await?;
    let has_ap_index: bool = sqlx::query_scalar::<_, i64>(
        "SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS
           WHERE TABLE_SCHEMA = DATABASE()
             AND TABLE_NAME = 'agent_profiles'
             AND INDEX_NAME = 'idx_agent_profiles_updated'",
    )
    .fetch_one(pool)
    .await?
        > 0;
    if !has_ap_index {
        sqlx::query("CREATE INDEX idx_agent_profiles_updated ON agent_profiles(updated_at)")
            .execute(pool)
            .await?;
    }
...
impl AgentProfileStore for MysqlAgentProfileStore {
    ...
    async fn get(&self, id: &str) -> Result<Option<AgentProfile>, BoxError> {
        let row: Option<(String,)> =
            sqlx::query_as(r#"SELECT payload FROM agent_profiles WHERE id = ?"#)
                .bind(id)
                .fetch_optional(&self.pool)
                .await
                .map_err(StoreError::from)?;
    ...
    async fn upsert(&self, item: &AgentProfile) -> Result<(), BoxError> {
        ...
        sqlx::query(
            r#"INSERT INTO agent_profiles (id, payload, updated_at) VALUES (?, ?, ?)
               ON DUPLICATE KEY UPDATE
