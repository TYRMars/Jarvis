...
export class RequirementListTool implements Tool {
  ...
  async invoke(args: JsonValue): Promise<string> {
    ...
    if (typeof limitRaw === "number" && Number.isFinite(limitRaw)) {
      // Mirror the schema `minimum: 1` at runtime: a negative/zero limit would
      // otherwise reach `slice(0, limit)` and silently drop rows (e.g. -1 →
      // `slice(0, -1)` drops the last row) instead of being capped.
