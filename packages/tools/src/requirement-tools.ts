...
export class RequirementListTool implements Tool {
  ...
  async invoke(args: JsonValue): Promise<string> {
    ...
    if (typeof limitRaw === "number" && Number.isFinite(limitRaw)) {
      // Clamp to the schema's [1, MAX_LIST_LIMIT] bound at runtime. Without the
      // lower bound a negative `limit` reaches `slice(0, -N)`, which silently
      // drops the last N rows instead of capping — a silently-wrong result.
