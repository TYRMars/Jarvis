...
async function scanTodoComments(
  ...
  // Real paths of directories already walked. Symlinks are followed (unlike
  // `fs.find`, which skips them), so without this guard a directory symlink
  // cycle (`loop -> .`, `a/b -> a`) recurses forever → call-stack overflow,
  // crashing this always-on read-only tool. Keyed by `realpath` so a link and
  // its target resolve to the same entry.
  ...
  async function walk(dir: string, ig: Ignore): Promise<boolean> {
    // Break symlink cycles: resolve to the canonical path and skip any
    // directory (or link target) we've already descended into.
    let real: string;
    try {
