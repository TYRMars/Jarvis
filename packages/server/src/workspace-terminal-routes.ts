 ...
 * `undefined` when the value is absent, non-finite, or out of range. Shared by
 * both the `?cols=/?rows=` query path and the WS `resize` frame path so the two
 * entry points enforce identical bounds.
 */
export function clampDim(n: number | undefined): number | undefined {
