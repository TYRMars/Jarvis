...
export function registerAutoModeRoutes(
  ...
  app.post("/v1/auto-mode", (req, reply) => {
    ...
    if (typeof rawBody.max_retries === "number" && Number.isFinite(rawBody.max_retries)) {
      ...
      // A negative ceiling would make the tick guard `consecutiveFailed >= maxRetries`
      // true for every requirement (even 0 failures satisfies `0 >= -1`), silently
      // disabling the whole scheduler — reject it rather than store an inert override.
