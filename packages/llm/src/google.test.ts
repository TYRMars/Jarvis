...
test("intoChatResponse via complete: decodes text + functionCall, synthesises gem id", async () => {
  ...
  // Gemini reports STOP even for tool-call turns; the provider promotes it to
  // "tool_calls" so the core loop dispatches the call instead of halting.
...
test("accumulator: mixed text + functionCall promotes STOP to tool_calls", () => {
  const acc = new StreamAccumulator();
  ...
  assert.ok(finish.type === "finish");
  // A streamed Gemini tool-call turn also reports STOP; promote so the loop dispatches.
  assert.equal(finish.finish_reason, "tool_calls");
  const m = finish.message as Extract<Message, { role: "assistant" }>;
  assert.equal(m.content, "thinking...");
  assert.equal(m.tool_calls?.length, 1);
