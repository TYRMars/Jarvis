 ...
 * multi-byte character. The cap (`JARVIS_PROJECT_CONTEXT_BYTES`) is a *byte*
 * budget, so accounting must use UTF-8 byte lengths — `String.length` /
 * `.slice()` count UTF-16 code units and would overshoot ~3x for CJK-heavy
 * context files.
 */
