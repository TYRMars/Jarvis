...
class Listeners<E> {
  ...
  emit(event: E): void {
    ...
    // the loop; isolate faults so a throwing subscriber can't reject the
    // already-committed mutation or starve later listeners.
    for (const fn of [...this.#fns]) {
      try {
        fn(event);
      } catch {
        // Listener fault isolated; the write already succeeded.
      }
    }
  }
}
