...
//
// `ChannelInstance` + `ChannelInstanceStatus` are sourced from
// @jarvis/shared-types — the Node single source of truth for wire shapes
// (formerly the Rust `ts_rs` bindings under `src/types/generated/`). Local
// aliases keep the legacy import sites stable.

import { apiUrl } from "./api";
import type {
  ChannelInstance as WireChannelInstance,
  ChannelInstanceStatus,
} from "@jarvis/shared-types";

export type ChannelStatus = ChannelInstanceStatus;
export type ChannelInstance = WireChannelInstance;
